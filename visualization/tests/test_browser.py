...
class BrowserTests(unittest.TestCase):
    ...
    def test_chat_preview_and_reset(self):
        ...
        self.assertEqual(self.page.locator("#prompt").input_value(), "Diabetes")
        self.page.locator("#prompt").press("Enter")
        self.assertIn("isn’t connected", self.page.locator(".assistant-message").first.inner_text())
        self.assertEqual(self.page.locator(".assistant-examples [data-run-example]").count(), 3)
    ...
    def test_worked_example_flow(self):
        self.open()
        self.assertEqual(self.page.locator("#example-list [data-run-example]").count(), 3)
        self.page.locator('#example-chips [data-run-example="kdr"]').click()
        self.assertTrue(self.page.locator("#example-result").is_visible())
        self.assertFalse(self.page.locator("#result").is_visible())
        self.assertIn("Colorectal cancer", self.page.locator("#example-title").inner_text())
        self.assertIn("Colorectal cancer", self.page.locator("#conversation .user-message").inner_text())
        self.assertIn("16,299", self.page.locator("#example-panel").inner_text())
        self.page.locator('#example-pipeline [data-step="search"]').click()
        self.assertIn("mebendazole", self.page.locator("#example-panel").inner_text())
        self.page.locator('#example-pipeline [data-step="check"]').click()
        self.assertIn("0.950", self.page.locator("#example-panel").inner_text())
        self.page.locator("#prompt").fill("lung cancer")
        self.page.locator("#prompt").press("Enter")
        self.assertIn("Lung cancer", self.page.locator("#example-title").inner_text())
        self.assertEqual(self.page.locator("#example-pipeline .pipeline-step.unavailable").count(), 1)
        self.page.locator("#save-example").click()
        self.assertEqual(self.page.locator("[data-saved-result]").count(), 1)
        self.page.reload()
        self.page.locator("#example-result").wait_for()
        self.assertIn("Lung cancer", self.page.locator("#example-title").inner_text())
        self.assertEqual(self.page.locator("#save-example").get_attribute("aria-pressed"), "true")
        self.page.locator(".inline-action[data-open-example]").first.click()
        self.assertIn("Colorectal cancer", self.page.locator("#example-title").inner_text())
        self.assertEqual(self.errors, [])
        self.assertEqual(self.external, [])
